...
> **Status:** Living catalogue of every feature currently in the codebase, large
> and small. A feature is listed here only if it is **implemented in source**.
> Planned-but-not-built items belong in [`roadmap.md`](./roadmap.md); actionable
> work belongs in [`todo.md`](./todo.md). For architecture see
> [`overview.md`](./overview.md).
>
> **Legend:** ✅ implemented · ⚠️ implemented with known limitation · 🧪 covered by tests

Reconciled against `versionName 2.0.0`.
...
| 1.7 | Persistent notification with overlay controls | ✅ | `OverlayService.kt`, `AppConstants.kt` |
...
| # | Feature | Status | Source |
| --- | --- | --- | --- |
| 1.9  | Ban-phase panel with ban recommendations | ✅ | `overlay/BanPhaseContent.kt`, `advisor/BanRecommender.kt` |
| 1.10 | Pick-phase panel with ranked hero suggestions | ✅ | `overlay/PickPhaseContent.kt` |
| 1.11 | Trading-phase panel (hero swap guidance) | ✅ | `overlay/TradingPhaseContent.kt` |
| 1.12 | Final draft report panel | ✅ | `overlay/FinalReportContent.kt` |
| 1.13 | Shared draft panel scaffold | ✅ | `overlay/DraftPanel.kt` |
...
| 6.6 | Hero list / explorer (paged) | ✅ | `herolist/HeroListScreen.kt`, `HeroListViewModel.kt`, `HeroListState.kt` |
...
| # | Feature | Status | Source |
| --- | --- | --- | --- |
| 7.1 | Room database (heroes, draft_sessions, hero_pool) | ✅ | `data/local/database/AppDatabase.kt` |
...
| 7.7 | Draft session persistence (single write path) | ✅ | `SaveDraftSessionUseCase.kt`, `DraftSessionRepositoryImpl.kt` |
...
| 8.5 | Battery-optimisation exemption request | ✅ | manifest permission + wizard |
...
| 9.2 | Reusable themed components | ✅ | `presentation/common/components/` |
...
| 9.7 | Thread-safe date formatting (java.time) | ✅ | `utils/DateFormatter.kt` |
| 9.8 | `NetworkResult` wrapper for typed outcomes | ✅ | `utils/NetworkResult.kt` |
| 9.9 | Image loading via Coil 3 | ✅ | `common/components/HeroPortrait.kt`, `HeroGrid.kt` |
...
| 9.13 | BuildConfig-driven API base URL | ✅ | `app/build.gradle.kts` |
...
> and service lifecycle are largely untested — tracked in [`todo.md`](./todo.md).
