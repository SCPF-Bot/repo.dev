...
#           sh -c "bash gradlew"   (bash)   OR
#           sh -c "ksh gradlew"    (ksh)    OR
#           sh -c "zsh gradlew"    (zsh)
#
#   (2) This script attempts to find your JDK location and set JAVA_HOME.
#       If you encounter an 'Error: JAVA_HOME is not set correctly', ensure
#       that JAVA_HOME is set to a correct JDK 17 home directory.
#
##############################################################################
...
if ! "$cygwin" && ! "$darwin" && ! "$nonstop" ; then
    case $MAX_FD in #(
      max*)
        # In POSIX sh, ulimit -H -n is not available. Use 'ulimit -Hn' and accept failure.
    ...
    case $MAX_FD in  #(
      '' | soft) :;; #(
      *)
        # In POSIX sh, ulimit -n is not available. Use 'ulimit -n' and accept failure.
        # shellcheck disable=SC2039,SC3045
...
#   * put everything else in double quotes as well, to prevent misinterpreting
#     spaces.
set -- \
