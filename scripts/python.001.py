...
def main():
    ...
    print("📊 Phase 1: Analyzing codebase structure...")
    global_analysis = phase1_analysis(client, files[:50])
    ...
    for idx, rel_path in enumerate(files, 1):
        ...
        if content is None:
            continue
        new_content, error = review_and_fix_file_full(client, rel_path, content)
        if error:
        ...
        if new_content.strip() != content.strip():
            print(f"   ✏️  Changes detected – applying")
